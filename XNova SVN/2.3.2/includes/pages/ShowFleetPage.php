...
{
	global $lang, $reslist, $resource,$db,$displays;

	//$parse				= $lang;
$displays->assignContent('fleet/fleet_table');
	$maxfleet  			= $db->query("SELECT COUNT(fleet_owner) AS `actcnt` FROM {{table}} WHERE `fleet_owner` = '".$CurrentUser['id']."';", 'fleets', true);
	...
	else
	{
		$page .= "<th colspan=\"2\"><a href=\"javascript:noShips();\" >".$lang['fl_remove_all_ships']."</a></th>";
                $page .= "<th colspan=\"2\"><a href=\"#\" onclick=\" ";//javascript:maxShips();\" >".$lang['fl_select_all_ships']."</a></th>";
