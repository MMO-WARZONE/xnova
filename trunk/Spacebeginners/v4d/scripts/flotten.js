...
function maxResource(id) {
	...
	var appoloniumToTransport = parseInt(document.getElementsByName("resource4")[0].value);
    ...
    if (isNaN(appoloniumToTransport)){
		appoloniumToTransport=0;
	}
	
	var freeCapacity = Math.max(storCap - metalToTransport - crystalToTransport - deuteriumToTransport - appoloniumToTransport, 0);
...
function maxResources() {
    ...
    var appoloniumToTransport = document.getElementsByName("thisresource4")[0].value;
	
	var freeCapacity = storCap - metalToTransport - crystalToTransport - deuteriumToTransport - appoloniumToTransport;
	if (freeCapacity < 0) {
		...
		appoloniumToTransport = Math.min(appoloniumToTransport, storCap - metalToTransport - crystalToTransport - deuteriumToTransport);
	}
	...
	document.getElementsByName("resource4")[0].value = Math.max(appoloniumToTransport, 0);
	calculateTransportCapacity();
}
...
function calculateTransportCapacity() {
	...
	var appolonium = Math.abs(document.getElementsByName("resource4")[0].value);

	transportCapacity =  storage() - metal - crystal - deuterium - appolonium;
...
function showResources() {
	...
	document.getElementsByName('resource4')[0].disabled = false;
...
function hideResources() {
	...
	document.getElementsByName('resource4')[0].disabled = true;
