...
doquery("UPDATE {{table}} SET `eingeladen` = '".mysql_real_escape_string($new_eingeladen_mr)."' WHERE `id` = '".$fleet['fleet_group']."' LIMIT 1 ;",'aks') or die($lang['fl_Add_member_to_fleet'].": <br />".mysql_error());
 ...
 $add_user_message_mr = "<font color=\"red\">".$lang['fl_error']." ".$lang['fl_player']." ".$_POST['addtogroup']." ".$lang['fl_dont_exist'].".";
    ...
    if (!isset($_POST['send'])) {
		...
		`planet_type` = '" . $fleet['fleet_end_type'] . "' AND
        `eingeladen` = '" . $user['id'] . "'
        ", 'aks',true);
 ...
 //    $aks_madnessred = mysql_query($aks_madnessred);
//  echo $aks_madnessred,$aks;
    ...
    */
    ...
    while ($f = mysql_fetch_array($fq)) {
      ...
      <input name=\"zawracanie\" value=" . $f['fleet_id'] . " type=hidden>
         <input value=\" Zurueck \" type=\"submit\">
       </form></th>";
        } else $page .= "&nbsp;</th>";
