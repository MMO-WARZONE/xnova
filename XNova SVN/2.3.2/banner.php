...
header ("Content-type: image/png");
 $image  = imagecreatefrompng("styles/banner/bann.png");
   // Parameters
   ...
   $querybase=$dbsettings['prefix']."users as u
             INNER JOIN ".$dbsettings['prefix']."statpoints as s
             ON s.id_owner=u.id AND s.stat_code=1 AND s.stat_type=1
             INNER JOIN ".$dbsettings['prefix']."planets as p
             ON p.id_owner=u.id AND p.planet_type=1
             INNER JOIN ".$dbsettings['prefix']."config as g
             ON g.config_name='users_amount'";
   $querywhere="u.id ='".$id."'";
