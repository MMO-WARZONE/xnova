...
function ProductionRates ($CurrentUser,$CurrentPlanet,$basic=false) {
	...
	*/
	...
	if($basic){
		...
		//2 Check storage
		$Caps['metal_max']	 = ((BASE_STORAGE_SIZE ) * floor((pow(1.6,$CurrentPlanet[$resource[22]]))* (1 + ($CurrentUser['rpg_stockeur'] * 0.5))));
		$Caps['crystal_max']   =((BASE_STORAGE_SIZE ) * floor((pow(1.6,$CurrentPlanet[$resource[23]]))* (1 + ($CurrentUser['rpg_stockeur'] * 0.5))));
		$Caps['deuterium_max'] =((BASE_STORAGE_SIZE ) * floor((pow(1.6,$CurrentPlanet[$resource[24]]))* (1 + ($CurrentUser['rpg_stockeur'] * 0.5))));
		$Caps['appolonium_max'] = ((BASE_STORAGE_SIZE) * floor((pow(1.6,$CurrentPlanet[$resource[25]]))* (1 + ($CurrentUser['rpg_stockeur'] * 0.5))));
		foreach ($resources as $res){
