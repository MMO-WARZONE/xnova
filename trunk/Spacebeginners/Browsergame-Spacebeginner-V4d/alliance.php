 ...
 */
        ...
        function bbtohtml($string){
            /*
            * Hier stehen die BB Codes 
            */
                $bbcode = array(   
                        '{\[b\](.*)\[/b\]}smUi',
                        '{\[u\](.*)\[/u\]}smUi',
               ...
               */
                $htmlcode = array(   
                        ...
                        '<blockquote>>Quote:<br />\\1</blockquote>',
            ...
            $string = preg_replace($bbcode,$htmlcode,$string);
            $string = str_replace('http://http://','http://',$string); 
            $string = str_replace('https://https://','http://',$string); 
...
*/
        ...
        if ($mode == 'exit') {
                ...
                // se sale de la alianza
                 if ($_GET['yes'] == 1) {
                         ...
                         doquery("UPDATE {{table}} SET `id_ally`= 0 WHERE `id_owner`='{$user['id']}'", "statpoints");
        ...
        */
              ...
              if($_POST['r'] === 0) {
              doquery("UPDATE {{table}} SET `new_message`= '+1' WHERE ally_id='{$user['ally_id']}'", "users");
              doquery("UPDATE {{table}} SET `mnl_alliance`= '+1' WHERE ally_id='{$user['ally_id']}'", "users");
              }else{
        ...
        */
              ...
              if ($u['ally_id'] == $ally['id'] && $u['id'] != $ally['ally_owner']) {
                ...
                doquery("UPDATE {{table}} SET `id_ally`= 0 WHERE `id_owner`='{$u['id']}'", "statpoints");
                doquery("UPDATE {{table}} SET `ally_members` = `ally_members` - 1 WHERE `id`='{$ally['id']}'", "alliance");
            }
