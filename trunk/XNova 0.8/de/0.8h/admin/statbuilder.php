...
//Schildkuppeln größer als 1 zurücksetzen
 ...
 //   doquery ( "UPDATE {{table}} SET `gig_protection_shield` = '1' WHERE `gig_protection_shield` > 1;" , 'planets');

	// Rotation des statistiques

	// x Tage bis zur löschung
	   $zeit = time()-60*60*24*14;
	...
	while ($CurUser = mysql_fetch_assoc($GameUsers)) {
		...
		$OldStatRecord  = doquery("SELECT * FROM {{table}} WHERE `id_owner` = '".$CurUser['id'].";'",'statpoints');  //, true wieder entfernt, dan die die ranglistendiffrentz net mehr
...
//}
		...
		$TTechPoints    = ($Points['TechPoint'] / 1000000);
		...
		while ($CurPlanet = mysql_fetch_assoc($UsrPlanets) ) {
			...
			$PlanetPoints     = $Points['BuildPoint'] / 1000000;
			$TBuildPoints    += ($Points['BuildPoint'] / 1000000);

			$Points           = GetDefensePoints ( $CurPlanet );
			$TDefsCount      += $Points['DefenseCount'];;
			$GCount          += $Points['DefenseCount'];
			$PlanetPoints    += ($Points['DefensePoint'] / 1000000);
			$TDefsPoints     += ($Points['DefensePoint'] / 1000000);

			$Points           = GetFleetPoints ( $CurPlanet );
			$TFleetCount     += $Points['FleetCount'];
			$GCount          += $Points['FleetCount'];
			$PlanetPoints    += ($Points['FleetPoint'] / 1000000);
			$TFleetPoints    += ($Points['FleetPoint'] / 1000000);
