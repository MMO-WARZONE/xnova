   ...
   message($lang['Urlaub01'], $lang['Urlaub02']);
        ...
        if ($_POST['ress'] != '') {
                ...
                if ($CheatTry  == false) {
                        switch ($_POST['ress']) {
                                case 'metal':
                                        $Necessaire   = (( $Crystal * 2) + ( $Deuterium * 4)+ ( $Appolonium * 8));
                                ...
                                case 'cristal':
                                        $Necessaire   = (( $Metal * 0.5) + ( $Deuterium * 2)+ ( $Appolonium * 4));
                                ...
                                case 'deuterium':
                                        $Necessaire   = (( $Metal * 0.25) + ( $Crystal * 0.5)+ ( $Appolonium * 2));
								 ...
								 case 'appolonium':
                                        $Necessaire   = (( $Metal * 0.125) + ( $Crystal * 0.25)+ ( $Deuterium * 0.5));
                                        if ($CurrentPlanet['appolonium'] > $Necessaire) {
                                                $CurrentPlanet['appolonium'] -= $Necessaire;
                                        } else {
                                                $Message = $lang['mod_ma_noten'] ." ". $lang['Appolonium'] ."! ";
                                                $Error   = true;
                                        }
                                        break;
                        }
                ...
                if ($_POST['action'] != 2) {
                        ...
                        switch ($_POST['choix']) {
                                ...
                                case 'deut':
										...
										$parse['mod_ma_res_c'] = "2";
                                        break;
								 case 'appolo':
                                        $PageTPL = gettemplate('marchand_appolonium');
