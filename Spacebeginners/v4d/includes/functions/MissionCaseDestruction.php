        ...
        foreach ($defense as $fleetID => $defender) { // Flottes et Défenses de la cible
            ...
            doquery( $QryUpdateTarget , 'planets');
                   ...
                   //Mondzerstörungswarscheinlichkeitsberechnung gemäß Owiki
                     ...
                     if ($attackFleets[$FleetRow['fleet_id']]['detail'][218] >= MAX_ST){
                      $destructionl2 = round($destructionl1 * sqrt(($attackFleets[$FleetRow['fleet_id']]['detail'][218]/(MoonDestroyMulti*$game_config['game_speed']))*17,5));
                       }
                       }
                      if ($attackFleets[$FleetRow['fleet_id']]['detail'][214] > 0 ){
                      if ($attackFleets[$FleetRow['fleet_id']]['detail'][214] >= MAX_RIP){
                     $destructionl2 = $destructionl2 + round($destructionl1 * sqrt(($attackFleets[$FleetRow['fleet_id']]['detail'][214]/(MoonDestroyMulti*$game_config['game_speed']))));
                      }
                     }
