...
if($_GET['img'] == 'good'){
	...
	$ExistUser = doquery("SELECT `username` FROM {{table}} WHERE `username` = '". mysql_real_escape_string($username) ."' LIMIT 1;", 'users', true);
	if(!$_GET['username']){
		$good = false;
	}elseif($ExistUser){
		$good = false;
	...
	$ExistEmail = doquery("SELECT `email` FROM {{table}} WHERE `email` = '". mysql_real_escape_string($email) ."' LIMIT 1;", 'users', true);
