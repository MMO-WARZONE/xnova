...
//Generic
        ...
        */
        ...
        $FleetDebris      = $result['debree']['att'][0] + $result['debree']['def'][0] + $result['debree']['att'][1] + $result['debree']['def'][1];
         ...
         $MoonChance       = $FleetDebris / (100000*$game_config['game_speed']);
               if ($FleetDebris > (2000000*$game_config['game_speed'])) {
                $UserChance = mt_rand(1, 100);
                $MoonChance = 20;
        } elseif ($FleetDebris < (100000*$game_config['game_speed'])) {
                $UserChance = 0;
                $ChanceMoon = "";
        } elseif ($FleetDebris >= (100000*$game_config['game_speed'])) {
                $UserChance = mt_rand(1, 100);
        ...
        $no_fleet = "<table border=1 align=\"center\"><tr><th>Type</th></tr><tr><th>Total</th></tr><tr><th>Waffen</th></tr><tr><th>Schilde</th></tr><tr><th></th>Panzerung</tr></table>";
        ...
        $report = str_replace($str_replace1, $str_replace2, $report);
        $Page .= $report;
    // Berechnung Trümmerfeld
        $Recab =  ceil( $FleetDebris/ $pricelist[209]['capacity']);
        $Gigarecab = ceil( $FleetDebris/ $pricelist[219]['capacity']);
        $Page .= "<br /><br />";
        $Page .= "Es werden <font color='red'>".$Recab."</font> Recycler für den TF-Abau benötigt!";
        $Page .= "<br /><br />";
        $Page .= "Es werden <font color='red'>".$Gigarecab."</font> Gigarecycler für den TF-Abau benötigt!";
        $Page .= "<br /><br />";
        // Berechnung und Anzeige Ende
        //We we aren't gonna chare this reoprt because we cheated so it acutally doesn't exist.
        $Page .= "Entschuldigung dieser Kampfbericht wird nicht gespeichert.";
        $Page .= "<br /><br />";
        $Page .= "</center>";
        $Page .= "</body>";
        $Page .= "</html>";
...
// Now its Sonyedorlys input form. Many thanks for allowing me to use it. (Slightly edited)
        ...
        if($user['military_tech'] != '') $parse['military']= $user['military_tech']  + $user['rpg_amiral'] * 0.5;
        if($user['shield_tech'] != '') $parse['shield'] = $user['shield_tech'] + $user['rpg_amiral'] * 0.5;
        if($user['defence_tech'] != '') $parse['defence'] = $user['defence_tech'] + $user['rpg_amiral'] * 0.5;
        ...
        $page .= "<table border=1 width=100%><tr><td class=\"c\">&nbsp;</td><td class=\"c\">Angreifer</td><td class=\"c\">Verteidiger + Admiral * 0,5</td></tr>";
        ...
        $page .= "<tr><th>Schilde</th><th><input type='text' name='shield_tech_us[1]' value='".$parse['shield']."'></th><th><input type='text' name='shield_tech_them[0]' value='".$parse[ 110 ]."'></th></tr>";
        $page .= "<tr><th>Panzerung</th><th><input type='text' name='defence_tech_us[1]' value='".$parse['defence']."'></th><th><input type='text' name='defence_tech_them[0]' value='".$parse[ 111 ]."'></th></tr>";
