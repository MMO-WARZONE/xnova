	...
	foreach ($FleetArray as $Ship => $Count) {
		if ($Ship == 202) {
			if ($Player['impulse_motor_tech'] >= 5) {
				$speedalls[$Ship] = $pricelist[$Ship]['speed2'] + (($pricelist[$Ship]['speed'] * $Player['impulse_motor_tech']) * 0.2) + ($pricelist[$Ship]['speed'] * $Player['rpg_conq']);
			} else {
				$speedalls[$Ship] = $pricelist[$Ship]['speed']  + (($pricelist[$Ship]['speed'] * $Player['combustion_tech']) * 0.1) + ($pricelist[$Ship]['speed'] * $Player['rpg_conq']);
			}
		}
		...
		if ($Ship == 205 or $Ship == 206 or $Ship == 208) {
			$speedalls[$Ship] = $pricelist[$Ship]['speed'] + (($pricelist[$Ship]['speed'] * $Player['impulse_motor_tech']) * 0.2) + ($pricelist[$Ship]['speed'] * $Player['rpg_conq']);
		}
		if ($Ship == 211) {
			if ($Player['hyperspace_motor_tech'] >= 8) {
				$speedalls[$Ship] = $pricelist[$Ship]['speed2'] + (($pricelist[$Ship]['speed'] * $Player['hyperspace_motor_tech']) * 0.3) + ($pricelist[$Ship]['speed'] * $Player['rpg_conq']);
			} else {
				$speedalls[$Ship] = $pricelist[$Ship]['speed']  + (($pricelist[$Ship]['speed'] * $Player['impulse_motor_tech']) * 0.2) + ($pricelist[$Ship]['speed'] * $Player['rpg_conq']);
			}
		}
		if ($Ship == 207 or $Ship == 213 or $Ship == 214 or $Ship == 215 or $Ship == 216 or $Ship == 217 or $Ship == 218) {
			$speedalls[$Ship] = $pricelist[$Ship]['speed'] + (($pricelist[$Ship]['speed'] * $Player['hyperspace_motor_tech']) * 0.3) + ($pricelist[$Ship]['speed'] * $Player['rpg_conq']);
		}
	}
	if ($Fleet != 0) {
...
function CreateFleetPopupedMissionLink ( $FleetRow, $Texte, $FleetType ) {
	...
	$FleetTotalC  = $FleetRow['fleet_resource_metal'] + $FleetRow['fleet_resource_crystal'] + $FleetRow['fleet_resource_deuterium'] + $FleetRow['fleet_resource_tachyon'];
	if ($FleetTotalC <> 0) {
		...
		$FRessource  .= "<tr><td width=50% align=left><font color=white>". $lang['Tachyon'] ."<font></td><td width=50% align=right><font color=white>". pretty_number($FleetRow['fleet_resource_tachyon']) ."<font></td></tr>";
		$FRessource  .= "</table>";
	} else {
		$FRessource   = "";
	}
