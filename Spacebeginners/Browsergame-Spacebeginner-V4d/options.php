       ...
       if (isset($_POST["exit_modus"]) && $_POST["exit_modus"] == 'on' and $user['urlaubs_until'] <= time()){
          ...
          doquery("UPDATE {{table}} SET
             ...
             WHERE `id` = '".$user['id']."' LIMIT 1", "users");
				...
				appolonium_perhour = '{$id['appolonium_perhour']}',
				...
				appolonium_mine_porcent = '{$id['appolonium_mine_porcent']}',
...
// Anfang Hintergrund bei Speicherung
$dpath = (!$user["dpath"]) ? DEFAULT_SKINPATH : $user["dpath"];
// Ende Hintergrund bei Speicherung

message($lang['succeful_save'], Einstellung ,"options.php",1);
} else {
$parse = $lang;
       ...
       if($user['urlaubs_modus']){
          ...
          display(parsetemplate(gettemplate('options_body_vmode'), $parse));
       }else{
       display(parsetemplate(gettemplate('options_body'), $parse));
       }
       die();
    }
