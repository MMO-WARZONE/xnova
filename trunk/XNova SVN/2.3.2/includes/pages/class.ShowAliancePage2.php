...
/**
 * Description of classShowAliancePage2
 *
 * @author Juan
 */
class classShowAliancePage2 {
	...
	private function MessageForm($Title, $Message, $Goto = '', $Button = ' OK ', $TwoLines = false)
	{
        ...
        private $mode,$a,$sort1,$sort2,$edit,$d, $rank, $kick,$id,$yes,$allyid,$show,$sendmail,$t,$tag;
        
        private function  gets(){
		$this->mode =(!empty($_GET["mode"]))?$_GET["mode"]:NULL;
                $this->a    =(!empty(intval($_GET['a'])))? intval($_GET['a']):NULL;
                $this->sort1=(!empty(intval($_GET['sort1'])))? intval($_GET['sort1']):NULL;
                $this->sort2=(!empty(intval($_GET['sort2'])))? intval($_GET['sort2']):NULL;
		$this->d    =(!empty(is_numeric($_GET['d'])))? $_GET['d']:NULL;
		$this->edit =(!empty($_GET['edit']))? $_GET['edit']:NULL;
		$this->rank =(!empty(intval($_GET['rank'])))? intval($_GET['rank']):NULL;
                $this->kick =(!empty(intval($_GET['kick'])))? intval($_GET['kicka']):NULL;
                $this->id   =(!empty(intval($_GET['id'])))? intval($_GET['ida']):NULL;
                $this->yes      = $_GET['yes'];
		$this->allyid   = intval($_GET['allyid']);
		$this->show     = intval($_GET['show']);
		$this->sendmail = intval($_GET['sendmail']);
		$this->t        = $_GET['t'];
		$this->tag      = mysql_escape_string($_GET['tag']);

        }
        private function visita_alianza(){
	...
	public function ShowAlliancePage($CurrentUser)
	{
		global $dpath, $phpEx, $lang,$db,$displays,$users;
                $this->gets();
                /*$displays->assignContent("alliance/alliance.tpl");
		...
		if ($_GET['mode'] == 'ainfo')
		{
			...
			//$displays->display(parsetemplate(gettemplate('alliance/alliance_ainfo'), $parse));
		}
		...
		{	//CREAR ALIANZA
			if ($mode == 'make' && $CurrentUser['ally_request'] == 0)
			{
				...
				{
					if (!$_POST['atag']){
						$diplays->message($lang['al_tag_required'], "game.php?page=alliance&mode=make",2);
					}

					if (!$_POST['aname']){
						$diplays->message($lang['al_name_required'],"game.php?page=alliance&mode=make",2);
					}
					...
					if ($tagquery){
						$diplays->message(str_replace('%s', $_POST['atag'], $lang['al_already_exists']),"game.php?page=alliance&mode=make",2);
					}
					...
					WHERE `id`='{$CurrentUser['id']}'", "users");
                                        ...
                                        $displays->display->newblock("fundar");
                                }
				$displays->display();
			}
			//BUSCAR ALIANZA
			if ($mode == 'search' && $CurrentUser['ally_request'] == 0)
			{
				//$page = parsetemplate(gettemplate('alliance/alliance_searchform'), $parse);
                                $displays->display->newblock("buscar");
				if ($_POST)
				{
					...
					{
                                                $displays->display->newblock("buscar_encontrar");
						while ($s = mysql_fetch_array($search))
						{
                                                        $displays->display->newblock("buscar_encontrado");
                                                        $displays->display->assign('ally_tag',"<a href=\"game.php?page=alliance&mode=apply&allyid={$s['id']}\">{$s['ally_tag']}</a>");
                                                        $displays->display->assign('ally_name',$s['ally_name']);
                                                        $displays->display->assign('ally_members',$s['ally_members']);
			...
			{ //SOLICITUDES
					...
					if ($_POST['enviar'] == $lang['al_applyform_send'])
					{
						...
						$displays->message($lang['al_request_confirmation_message']);
                                            ...
                                            $displays->display->newblock("solicitud");
			...
			if ($CurrentUser['ally_request'] != 0)
			{
				...
				extract($allyquery);
                                $displays->display->newblock("esp_solicitud");
				if ($_POST['bcancel'])
				{
			...
			{
                                $displays->display->newblock("default");
                                $displays->display();
			}
		}
		...
		// < LISTA DE MIEMBROS >
			if ($mode == 'memberslist')
			{
				...
				$i = 0;
                                $displays->display->newblock("memberslist");

				while ($u = mysql_fetch_array($listuser))
				{
                                        $displays->display->newblock("list_memberslist");
					$i++;
					$u['i'] = $i;
					...
					if ($u['ally_register_time'] > 0)
						...
						$u['ally_register_time'] = "-";
                                        ...
                                        foreach($u as $key => $value){
                                            $displays->display->assign($key,$value);
                                        }
				}
		...
		// < CORREO CIRCULAR >
			if ($mode == 'circular')
			{
				...
				//$displays->display(parsetemplate(gettemplate('alliance/alliance_circular'), $lang));
			}
		...
		// <  EDICIONES GENERALES DE LA ALIANZA >
			if ($mode == 'admin' && $edit == 'ally')
			{
				...
				elseif ($_POST['t'])
				{
					...
					elseif ($t == 2)
					{
						$ally['ally_text'] = mysql_escape_string(strip_tags($this->bbcode($_POST['text'])));
						$db->query("UPDATE {{table}} SET
					...
					else
					{
						...
						WHERE `id`='{$ally['id']}'", "alliance");
                                ...
                                $displays->display->newblock("admin");
			...
			if ($mode == 'admin' && $edit == 'name')
			{
				if ($ally['ally_owner'] != $CurrentUser['id'] && !$user_admin)
					header("location:game.". $phpEx . "?page=alliance",2);
                                ...
                                $displays->display->newblock("rename_alliance");
		...
		// < - CAMBIAR ETIQUETA DE LA ALIANZA - >
			if ($mode == 'admin' && $edit == 'tag')
			{
				...
				if ($_POST['etiqueta'] && !empty($_POST['etiqueta'])){
					$db->query("UPDATE {{table}} SET `ally_tag` = '". mysql_escape_string(strip_tags($_POST['etiqueta'])) ."' WHERE `id` = '". $CurrentUser['ally_id'] ."';", 'alliance');
                                }
		...
		// < -- PARTE DEFAULT DE LA ALIANZA -- >
			{
                                ...
                                $displays->display->newblock("alianza");
				// IMAGEN
				if ($ally['ally_ranks'] != '')
				...
				$lang['ally_text'] 		= $this->bbcode($ally['ally_text']);
                                $lang['ally_web']=($ally['ally_web'] != '')?$ally['ally_web']:"-";
				$lang['ally_tag'] 			= $ally['ally_tag'];
