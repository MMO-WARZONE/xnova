...
{
	global $lang, $phpEx,$db,$displays;

	$a = $_GET['a'];

        $displays->assignContent("fleet/shortcut");
	if (isset($_GET['mode']))
	{
		if ($_POST)
		{
			...
			$db->query("UPDATE {{table}} SET fleet_shortcut='{$CurrentUser[fleet_shortcut]}' WHERE id={$CurrentUser[id]}", "users");
