...
function AddPluginShip(&$AddedArray, &$lastShipKey, &$element, &$language, &$prices, &$need, &$combat, &$overflow = false){
	global $resource, $requeriments, $pricelist, $CombatCaps, $ProdGrid, $reslist, $lang;
	if($overflow === false){
		$lastShipKey++;
		$key = $lastShipKey;
	}else{
	...
	$query = doquery("DESCRIBE {{table}};", 'planets', false);
	$ExistsRecord = false;
	while($row = mysql_fetch_array($query)) {
		if($row['Field'] == $element){	
			$ExistsRecord = true;
			break;
		}
	}
	if(!$ExistsRecord){
		doquery("ALTER TABLE {{table}} ADD `".$element."` BIGINT(11) NOT NULL DEFAULT '0';", 'planets');
	}
	
	//empezamos a añadir todo :)
	$resource[$key] = $element;
	if($need !== false){
		$requeriments[$key] = $need;
	}
	$pricelist[$key] = $prices;
	$CombatCaps[$key] = $combat;
	if($overflow === false){
		$reslist['fleet'][] = $key;
	}
	$lang['tech'][$key] = $language['name'];
	$lang['res']['descriptions'][$key] = $language['description'];
	$lang['info'][$key]['name'] = $language['name'];
	$lang['info'][$key]['description'] = (isset($language['description_long'])) ? $language['description_long'] : $language['description'];
	if(isset($need[115])){
		$AddedArray[$key] = 115;
	}elseif(isset($need[117])){
		$AddedArray[$key] = 117;
	}elseif(isset($need[118])){
		$AddedArray[$key] = 118;
	}

	unset($element, $language, $prices, $need, $combat, $prod, $overflow);

}
//read new ships
$lastShipKey = 201;
foreach($reslist['fleet'] as $value){
$lastShipKey = ($value > $lastShipKey) ? $value : $lastShipKey;
}
$dir = opendir($plugins_path.'newships/');
...
while (($file = readdir($dir)) !== false)
{
	...
	if ($extension == '.xgship')
	{
		include $plugins_path.'newships/' . $file;
		AddPluginShip($PluginShips, $lastShipKey, $element, $language, $prices, $need, $combat, $overflow);
		
	}
	elseif (file_exists($plugins_path.$file.'/'.$file.'.'.$phpEx))
	{ // by the way, we check if the ship is inside of a folder
		include $plugins_path.'newships/'.$file.'/'.$file.'.'.$phpEx;
	}
}
ksort($lang['tech']);
closedir($dir);
