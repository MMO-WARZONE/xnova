...
$parse['who']    = "<option value=\"1\"". (($who == "1") ? " SELECTED" : "") ."> ". $lang['stat']['9900'] ."</option>"; // Alle Team-Mitglieder ohne User
$parse['who']   .= "<option value=\"2\"". (($who == "2") ? " SELECTED" : "") ."> ". $lang['stat']['9901'] ."</option>"; // Alle User vom Volk A
$parse['who']   .= "<option value=\"3\"". (($who == "3") ? " SELECTED" : "") ."> ". $lang['stat']['9902'] ."</option>"; // Alle User vom Volk B
$parse['who']   .= "<option value=\"4\"". (($who == "4") ? " SELECTED" : "") ."> ". $lang['stat']['9903'] ."</option>"; // Alle User vom Volk C
$parse['who']   .= "<option value=\"5\"". (($who == "5") ? " SELECTED" : "") ."> ". $lang['stat']['9904'] ."</option>"; // Alle User ohne Team-Mitglieder
$parse['who']   .= "<option value=\"6\"". (($who == "6") ? " SELECTED" : "") ."> ". $lang['stat']['9905'] ."</option>"; // Alle Allianzen

$parse['type']   = "<option value=\"1\"". (($type == "1") ? " SELECTED" : "") .">". $lang['stat']['9001'] ."</option>";
$parse['type']  .= "<option value=\"2\"". (($type == "2") ? " SELECTED" : "") .">". $lang['stat']['9002'] ."</option>";
$parse['type']  .= "<option value=\"3\"". (($type == "3") ? " SELECTED" : "") .">". $lang['stat']['9003'] ."</option>";
$parse['type']  .= "<option value=\"4\"". (($type == "4") ? " SELECTED" : "") .">". $lang['stat']['9004'] ."</option>";
$parse['type']  .= "<option value=\"5\"". (($type == "5") ? " SELECTED" : "") .">". $lang['stat']['9005'] ."</option>";
...
if ($who == 1) { // Teammitglieder
    $MaxUsers = doquery ("SELECT COUNT(*) AS `count` FROM {{table}} WHERE `db_deaktjava` = '0';", 'users', true);

    $dpath          = (!$user["dpath"]) ? DEFAULT_SKINPATH : $user["dpath"];
    $parse['dpath'] = $dpath;
    $parse['info_03'] = "<img src='./styl/image/stat/team.png' style='height:70px; width:197px;' alt=''>";
    ...
    $query = doquery("SELECT * FROM {{table}} WHERE `stat_type` = '6' AND `stat_code` = '1' ORDER BY `". $Order ."` DESC LIMIT ". $start .",100;", 'statpoints');
    ...
    while ($StatRow = mysql_fetch_assoc($query)) {
        ...
        $QryUpdateStats .= "`stat_type` = '6' AND `stat_code` = '1' AND `id_owner` = '". $TheRank['id_owner'] ."';";
        $rank_old                 = $StatRow[ $OldRank ];

        if ( $rank_old == 0) {
            $rank_old             = $start;
            $QryUpdRank           = doquery("UPDATE {{table}} SET `".$Rank."` = '".$start."', `".$OldRank."` = '".$start."' WHERE `stat_type` = '6' AND `stat_code` = '1' AND `id_owner` = '". $StatRow['id_owner'] ."';" , "statpoints");
        } else {
            $QryUpdRank           = doquery("UPDATE {{table}} SET `".$Rank."` = '".$start."' WHERE `stat_type` = '6' AND `stat_code` = '1' AND `id_owner` = '". $StatRow['id_owner'] ."';" , "statpoints");
        }
        ...
        $parse['info_01'] = "".$lang['stat']['0001_04']."";
    ...
    $dpath          = (!$user["dpath"]) ? DEFAULT_SKINPATH : $user["dpath"];
    $parse['dpath'] = $dpath;
    $parse['info_03'] = "<img src='./styl/image/stat/sulurior.png' style='height:70px; width:197px;' alt=''>";
    ...
    $parse['stat_header'] = parsetemplate(gettemplate('stat/stat_volk_02'), $parse);
    $start = floor($range / 100 % 100) * 100;
    $query = doquery("SELECT * FROM {{table}} WHERE `stat_type` = '4' AND `stat_code` = '1' ORDER BY `". $Order ."` DESC LIMIT ". $start .",100;", 'statpoints');
    ...
    while ($StatRow = mysql_fetch_assoc($query)) {
        ...
        $QryUpdateStats .= "`stat_type` = '4' AND `stat_code` = '1' AND `id_owner` = '". $TheRank['id_owner'] ."';";
        $rank_old                 = $StatRow[ $OldRank ];

        if ( $rank_old == 0) {
            $rank_old             = $start;
            $QryUpdRank           = doquery("UPDATE {{table}} SET `".$Rank."` = '".$start."', `".$OldRank."` = '".$start."' WHERE `stat_type` = '4' AND `stat_code` = '1' AND `id_owner` = '". $StatRow['id_owner'] ."';" , "statpoints");
        } else {
            $QryUpdRank           = doquery("UPDATE {{table}} SET `".$Rank."` = '".$start."' WHERE `stat_type` = '4' AND `stat_code` = '1' AND `id_owner` = '". $StatRow['id_owner'] ."';" , "statpoints");
        }
        ...
        $parse['info_01'] = "".$lang['stat']['0001_02']."";
        $parse['info_02'] = "".$lang['stat']['0002']."";
    ...
    $dpath          = (!$user["dpath"]) ? DEFAULT_SKINPATH : $user["dpath"];
    $parse['dpath'] = $dpath;
    $parse['info_03'] = "<img src='./styl/image/stat/galaktiker.png' style='height:70px; width:197px;' alt=''>";
    ...
    $parse['stat_header'] = parsetemplate(gettemplate('stat/stat_volk_02'), $parse);
    $start = floor($range / 100 % 100) * 100;
    $query = doquery("SELECT * FROM {{table}} WHERE `stat_type` = '5' AND `stat_code` = '1' ORDER BY `". $Order ."` DESC LIMIT ". $start .",100;", 'statpoints');
    ...
    while ($StatRow = mysql_fetch_assoc($query)) {
        ...
        $QryUpdateStats .= "`stat_type` = '5' AND `stat_code` = '1' AND `id_owner` = '". $TheRank['id_owner'] ."';";
        $rank_old                 = $StatRow[ $OldRank ];

        if ( $rank_old == 0) {
            $rank_old             = $start;
            $QryUpdRank           = doquery("UPDATE {{table}} SET `".$Rank."` = '".$start."', `".$OldRank."` = '".$start."' WHERE `stat_type` = '5' AND `stat_code` = '1' AND `id_owner` = '". $StatRow['id_owner'] ."';" , "statpoints");
        } else {
            $QryUpdRank           = doquery("UPDATE {{table}} SET `".$Rank."` = '".$start."' WHERE `stat_type` = '5' AND `stat_code` = '1' AND `id_owner` = '". $StatRow['id_owner'] ."';" , "statpoints");
        }
        ...
        $parse['info_01'] = "".$lang['stat']['0001_03']."";
        $parse['info_02'] = "".$lang['stat']['0002']."";
