...
function GetMaxConstructibleElements ($Element, $Ressources) {
	...
	if ($pricelist[$Element]['appolonium'] != 0) {
		$ResType_4_Needed = $pricelist[$Element]['appolonium'];
		$Buildable        = floor($Ressources["appolonium"] / $ResType_4_Needed);
	}
	if (!isset($MaxElements)) {
		$MaxElements      = $Buildable;
	} elseif ($MaxElements > $Buildable) {
	...
	if ($pricelist[$Element]['energy'] != 0) {
		$ResType_5_Needed = $pricelist[$Element]['energy'];
		$Buildable        = floor($Ressources["energy_max"] / $ResType_5_Needed);
	}
	if ($Buildable < 1) {
		$MaxElements      = 0;
	}
