...
} elseif ($FleetRow['fleet_mess'] == 1 && $FleetRow['fleet_end_stay']!=0) {

$Retour = $FleetRow['fleet_start_time']-$FleetRow['start_time'];

$H_Retour = time()+$Retour;

$QryUpdateFleet = "UPDATE {{table}} SET ";
$QryUpdateFleet .= "`fleet_start_time` = '". (time() - 1) ."', ";
$QryUpdateFleet .= "`fleet_end_stay` = '0', ";
$QryUpdateFleet .= "`fleet_end_time` = '". $H_Retour ."', ";
$QryUpdateFleet .= "`fleet_target_owner` = '". $user['id'] ."', ";
$QryUpdateFleet .= "`fleet_mess` = '1' ";
$QryUpdateFleet .= "WHERE ";
$QryUpdateFleet .= "`fleet_id` = '" . $fleetid . "';";
...
if ($FleetRow['fleet_mission'] == 5 or $FleetRow['fleet_mission'] == 15) {
if ($FleetRow['fleet_end_stay'] <= 0) {
// Faut calculer le temps reel de retour
if ($FleetRow['fleet_start_time'] < time()) {
// On a pas encore entamé le stationnement
...
// Open Source
// (c) by MoF
// Fix Halten Flottenrückkehr Team Spacebeginner 2010
?>
