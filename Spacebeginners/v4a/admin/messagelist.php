...
global $Adminerlaubt;
	...
	if ( $user['authlevel'] >= 2 and in_array ($user['id'],$Adminerlaubt) ) {
		...
		while ($row = mysql_fetch_assoc($Messages)) {
			$OwnerData = doquery ("SELECT `username`, authlevel  FROM {{table}} WHERE `id` = '". $row['message_owner'] ."' LIMIT 1;", 'users',true);
			$SenderData = doquery ("SELECT `username`, authlevel  FROM {{table}} WHERE `id` = '". $row['message_sender'] ."' LIMIT 1;", 'users',true);
			$bloc['mlst_id']      = $row['message_id'];
			$bloc['mlst_from']    = $row['message_from'];
			$bloc['mlst_to']      = $OwnerData['username'] ." ID:". $row['message_owner'];
			if($user['authlevel'] >= $OwnerData['authlevel'] AND $user['authlevel'] >= $SenderData['authlevel']){
				$bloc['mlst_text']    = $row['message_text'];
			}else{
				$bloc['mlst_text']    = "Besser, Sie lesen diese Nachrichten.";
			}
