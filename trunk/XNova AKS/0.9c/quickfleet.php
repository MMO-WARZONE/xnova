...
includeLang('fleet');
	...
	if (($Mode == 8) and $planetrow[$resource[219]] != 0){
		...
		$RecyclerNeeded   = floor($DebrisSize / ($pricelist[219]['capacity'])) + 1;
		$RecyclerSpeed    = $pricelist[219]['speed'] + (($pricelist[219]['speed'] * $user['combustion_tech']) * 0.1);

		$RecyclerCount    = $planetrow[$resource[219]];
		if ($RecyclerCount > $RecyclerNeeded) {
			$FleetCount = $RecyclerNeeded;
		} else {
			$FleetCount = $RecyclerCount;
		}
		$FleetArray[219] = $FleetCount;
	    }
        elseif (($Mode == 8) and $planetrow[$resource[209]] != 0){
		$QrySelectGalaxy  = "SELECT * FROM {{table}} ";
		$QrySelectGalaxy .= "WHERE ";
		$QrySelectGalaxy .= "`galaxy` = '".$planetrow['galaxy']."' AND ";
		$QrySelectGalaxy .= "`system` = '".$planetrow['system']."' AND ";
