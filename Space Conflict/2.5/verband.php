...
include($xnova_root_path . 'common.' . $phpEx);
	...
	if($_POST['add_member_to_aks'] == "madnessred"){
		...
		if($added_user_id_mr > 0){
			...
			doquery("UPDATE {{table}} SET `eingeladen` = '".$new_eingeladen_mr."' ;",'aks') or die("Adding member to fleet: <br />".mysql_error());
		...
		// Send a message.
		$invite_message = "Player ".$user['username']." has invited you to join and ACS attack. You can join this attack from the 'Fleet' page.";
	...
	if (mysql_num_rows($query) != 1) {
		message('That fleet doesn\'t exist (any more)!', 'Error');
	}
...
<script language="JavaScript" src="scripts/ocnt.js"></script>
  <center>
	...
	*/
    ...
    <table width="519" border="0" cellpadding="0" cellspacing="1">
   <tr height="20">
     <td class="c" colspan="2">	Association de flotte '.$aks_code_mr.'</td>
   </tr>
