...
function CreateOneMoonRecord ( $Galaxy, $System, $Planet, $Owner, $MoonName, $Chance )
{
	...
	$MoonGalaxy = $db->query ( $QryGetMoonGalaxyData, 'galaxy', true);
	...
	if ($MoonGalaxy['id_luna'] == 0)
	{
		if ($MoonPlanet['id'] != 0 && $MoonPlanet['planet_type'] != 3)
		{
			...
			$lunarow = $db->query( $QryGetMoonIdFromPlanet, 'planets', true);
			...
			$db->query( $QryUpdateMoonInGalaxy , 'galaxy');
