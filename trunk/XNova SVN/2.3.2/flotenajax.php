...
$PrNoob      = $db->game_config['noobprotection'];
$PrNoobTime  = $db->game_config['noobprotectiontime'];
$PrNoobMulti = $db->game_config['noobprotectionmulti'];
...
$CurrentFlyingFleets = $db->query("SELECT COUNT(fleet_id) AS `Nbre` FROM {{table}} WHERE `fleet_owner` = '".$users->user['id']."';", 'fleets', true);
...
$TargetRow = $db->query( $QrySelectEnemy, 'planets', true);
if ($TargetRow['id_owner'] == '')
	$TargetUser = $users->user;
elseif ($TargetRow['id_owner'] != '')
	$TargetUser = $db->query("SELECT * FROM {{table}} WHERE `id` = '". $TargetRow['id_owner'] ."';", 'users', true);


$UserPoints    = $db->query("SELECT * FROM {{table}} WHERE `stat_type` = '1' AND `stat_code` = '1' AND `id_owner` = '". $users->user['id'] ."';", 'statpoints', true);
$User2Points   = $db->query("SELECT * FROM {{table}} WHERE `stat_type` = '1' AND `stat_code` = '1' AND `id_owner` = '". $TargetUser['id'] ."';", 'statpoints', true);
...
if ((($users->user[$resource[108]] + 1) + ($users->user['rpg_commandant'] * 3)) <= $CurrentFlyingFleets)
{
...
$noobs->prin=$CurrentPoints;
$noobs->mission=$_POST['mission'];
$noobs->seco=$TargetPoints;
$noobs->vacation=$TargetUser['urlaubs_modus'];
$noobs_users=$noobs->check();
if($noobs_users['status']){
        $span=explode("_", $noobs_users['lang_g']);
        $array=array("s"=>604,"w"=> 603,"v"=>620);
...
$speedall    = GetFleetMaxSpeed ($FleetArray, 0, $users->user);
...
if ($TargetRow['id_level'] > $users->user['authlevel'])
{
	$Allowed = true;
...
$QryInsertFleet .= "`fleet_owner` = '". $users->user['id'] ."', ";
...
$planetrow 		= $db->query("SELECT * FROM {{table}} WHERE `id` = '". $user['current_planet'] ."';", 'planets', true);
