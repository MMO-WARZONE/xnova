...
function BuildRessourcePage ( $CurrentUser, $CurrentPlanet ) {
	...
	$parse['energy_total']          = colorNumber( pretty_number( floor( ( $CurrentPlanet['energy_max'] * 0.01 * (1 + $CurrentUser['energy_tech'] * 0.01) + $parse['energy_basic_income'] ) + $CurrentPlanet['energy_used'])));
	$parse['energy_tech_bonus']     = colorNumber( pretty_number( floor( ( $CurrentPlanet['energy_max'] + $parse['energy_basic_income'] ) * 0.01 * (1 +  $CurrentUser['energy_tech'] * 0.01) - (($CurrentPlanet['energy_max'] + $parse['energy_basic_income']) * 0.01))));
	$parse['energy_max']            = colorNumber(pretty_number($CurrentPlanet['energy_max'] * 0.01 * (1 + $CurrentUser['energy_tech'] * 0.01))) ;
