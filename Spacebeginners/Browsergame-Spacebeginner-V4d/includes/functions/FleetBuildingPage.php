				...
				if ( IsTechnologieAccessible ($CurrentUser, $CurrentPlanet, $Element) ) {
                   ...
                   if ($Count >= 1) {
                      ...
                      $CurrentPlanet['deuterium']         -= $Ressource['deuterium'];
					   $CurrentPlanet['appolonium']       -= $Ressource['appolonium'];
					  if($Element == 214) {                        
			...
			if (IsTechnologieAccessible($CurrentUser, $CurrentPlanet, $Element)) {
                ...
                if ($CanBuildOne) {
                   ...
                   if ($BuildOneElementTime > 0) {
                      $PageTable .= "<input type=text id=fmenge[".$Element."] name=fmenge[".$Element."] alt='".$lang['tech'][$Element]."' size=10 maxlength=10 value=0 tabindex=".$TabIndex.">";
                   } else {
                      $PageTable .= "<input type=text id=fmenge[".$Element."] name=fmenge[".$Element."] alt='".$lang['tech'][$Element]."' size=10 maxlength=10 value=0 tabindex=".$TabIndex.">";
                   }
                ...
                if ($CanBuildOne)
                 $PageTable.='<BR><BR><A ONCLICK="document.getElementById(\'fmenge['.$Element.']\').value=\''.intval($MaxElements).'\';" STYLE="cursor:pointer;">Max ('.intval($MaxElements).')</A>';
