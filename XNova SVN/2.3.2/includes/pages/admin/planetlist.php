...
if ($_GET['cmd'] == 'sort') {
	...
	$short = 'id';
		...
		$parse = $lang;
		$query = doquery("SELECT `id`, `id_owner`, `name`, `last_update`,  `field_current`, `field_max`, `galaxy`, `system`, `planet` FROM {{table}} WHERE planet_type=1 ORDER BY `". $short ."`", "planets");
		...
		while ($u = mysql_fetch_array($query)) {
			...
			. "<td class=b><center><b><a href='planetlist.php?action=edit&id=".$u['id'] ."'>" . $u['field_max'] . "/" . $u['field_current'] . "</a></center></b></td>"
			. "<td class=b><center><b><a href='planetlist.php?action=edit&id=".$u['id'] ."'>" . $u['galaxy'] . ":" . $u['system'] . ":" . $u['planet'] . "</a></center></b></td>"
			. "<td class=b><center><b><a href='planetlist.php?action=edit&id=".$u['id'] ."'>" . pretty_time(time() - $u['last_update']) . "</a></center></b></td>"
			. "</tr>";
			$i++;
		}

		if ($i == "1")
			$parse['planetes'] .= "<tr><th class=b colspan=6>Hay 1 planeta colonizado</th></tr>";
		else
			$parse['planetes'] .= "<tr><th class=b colspan=6>Hay {$i} planetas colonizados</th></tr>";
		...
		if(isset($_POST['submit'])) {
			...
			AdminMessage ('<meta http-equiv="refresh" content="1; url=planetlist.php">Los datos han sido modificados', 'Edicion de planeta');
		}
